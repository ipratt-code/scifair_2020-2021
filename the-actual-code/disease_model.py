...
# These are the variables that we will tweak with lmfit
...
def deriv(y, t, N, beta, gamma, delta, alpha, rho, epsilon, full_output=1):
    S, E, I, R, D = y
...
S0, E0, I0, R0, D0 = (
    N - starting_infected - starting_dead,
    0,
    starting_infected,
    0,
    starting_dead,
...
t = np.linspace(0, how_long, how_long)  # Grid of time points (in days)
...
ret = odeint(deriv, y0, t, args=(N, beta, gamma, delta, alpha, rho, epsilon))
S, E, I, R, D = ret.T

data_array = [S, E, I, R, D]

np.savetxt(
    disease_name + "-pop_size:" + str(N) + ".csv", data_array, delimiter=",", fmt="%d"
)
...
ax.set_title("Effect of " + disease_name + " on a population of " + str(N))
